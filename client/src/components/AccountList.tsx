                ...
                <div key={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:border-slate-300 transition-all">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        {account.status === 'connected' ? (
                            ...
                            <>
                            <AlertCircleIcon className="sizze-4 text-amber-500"/>
                            <span className="text-xs text-amber-600">Disconnected</span>
                            </>
                        )}
                    </div>
                    <button onClick={()=> handleDisconnect(account._id)} title="Disconnect account" className="m-2 p-1.5 eounded-lg text-slate-300 group-hover:text-red-500 transition-all">
